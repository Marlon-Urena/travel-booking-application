...
public class DataBase {
    ...
    public TravelerAccount loginToTravelerAccount(String login) throws Exception, RuntimeException {
           ...
           inputEmail = login.substring(0, login.indexOf(" ")-1); // Looks at what's before the space
           ...
           travelerAccount = userMap.get(inputEmail);
               ...
               if (travelerAccount == null) {
                   throw new Exception(); //Would replace with custom exception for incorrect email
               }
               else if (travelerAccount.getLoginCredentials()== login){
                  successfulLogin = true;
               }
               else if (!successfulLogin) {
                   throw new RuntimeException(); //Would replace with custom exception for incorrect email and password combination
               }
